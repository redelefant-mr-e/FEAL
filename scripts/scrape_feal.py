...
def product_row(
    *,
    ...
    row = {
        ...
        "source_url_sv": source_url_sv,
        "source_url_en": source_url_en,
        "parent_page_sv": parent_page_sv,
        "parent_page_en": parent_page_en,
    }
    for i in range(1, 9):
        row[f"image_{i}"] = images[i - 1] if i <= len(images) else ""
    return row
...
def variant_row(
    *,
    ...
    return {
        ...
        "category_key": category_key,
        "article_number": art,
        "name_sv": merged["name_sv"],
        "name_en": merged["name_en"],
        "length_mm": merged.get("length_mm", ""),
        "width_mm": merged.get("width_mm", ""),
        "inside_width_mm": merged.get("inside_width_mm", ""),
        "folded_length_mm": merged.get("folded_length_mm", ""),
        "folded_height_mm": merged.get("folded_height_mm", ""),
        "weight_kg": merged.get("weight_kg", ""),
        "max_load_kg": merged.get("max_load_kg", ""),
        "rec_max_height_mm": merged.get("rec_max_height_mm", ""),
        "adjustable_height_mm": merged.get("adjustable_height_mm", ""),
        "angle_deg": merged.get("angle_deg", ""),
        "other_specs_sv": merged.get("other_specs_sv", ""),
        "other_specs_en": merged.get("other_specs_en", ""),
        "specs_raw_sv": merged.get("specs_raw_sv", ""),
        "specs_raw_en": merged.get("specs_raw_en", ""),
...
def apply_hosted_image_urls() -> None:
    ...
    if not url_map:
        return
    image_col = re.compile(r"^image(_\d+)?$", re.I)
    updated = 0
